...
int build_table_local( int nitems, int cap, int padCap, shared [BLK_SIZE] int *T, int *Tlocal, int *w, int *v )
{
    ...
    upc_memput((shared void *) (T+startIdx), (void *) (Tlocal+startIdx), min(BLK_SIZE, max(cap-w[1]-startIdx, 0))*sizeof(int)); // Can be changed to 0 to cap-w[j+1]
	...
	upc_memput((shared void *) (T+startIdx+padCap+1), (void *) (Tlocal+startIdx+padCap+1), min(BLK_SIZE, max(cap-w[j+1]-startIdx, 0))*sizeof(int));
